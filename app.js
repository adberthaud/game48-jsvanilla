...
function rollDices() {
  if (dCount < 5) {
    for (let dice of dices) {
      dice.style.animation = 'shakeDice 1.5s ease';
    }
    setTimeout(() => {
      diceResultGen();
    }, 1500); // Updating dice values at end of animation
    ableToRoll = false;
    ableToSelect = 2;
  } else {
    endOfTurn();
...
function diceResultGen() {
  ...
  if (dCount < 5) {
    const diceMath = (dValue) => {
      return Math.ceil(Math.random() * dValue);
    };
...
function finishSelection() {
  // END OF TURN LAYOUT FOR SELECTED DICES
  ableToRoll = true;
...
function newGameSetup() {
  // LAYOUT RESET TO INITIAL TURN SETUP + TRIGGER OF END OF GAME SCENARIO
...
function endOfRound() {
  if (tutorialOn === true && roundCount === 0) {
    setTimeout(() => {
      tutoScore();
    }, 2000);
  }
...
function dicesReset() {
  ...
  for (let diceSelected of dicesSelected) {
    diceSelected.style.opacity = 0;
  }
...
function pushScoresToTotal() {
  ...
  for (i = 0; i < playerTotalScore.length; i++) {
    let totalScore =
      parseInt(playerTotalScore[i].innerText) -
      parseInt(playerScoresArr[i].innerText);
    playerTotalScore[i].innerText = `${totalScore} points = ${(
      totalScore * euroParameter
    ).toFixed(2)}€`;
  }

  let winnerScore = newArray.reduce((a, b) => a + b, 0) - bestScore;
  let winnerScoreAccumulation =
    bestScore + winnerScore + parseInt(playerTotalScore[winnerIndex].innerText);
  playerTotalScore[
    winnerIndex
  ].innerText = `${winnerScoreAccumulation} points = ${(
    winnerScoreAccumulation * euroParameter
  ).toFixed(2)}€`;
...
function endOfGame(winnerIndex) {
  ...
  for (i = 0; i < playerTotalScore.length; i++) {
    let moneyWin = (
      parseInt(playerTotalScore[i].innerText) * euroParameter
    ).toFixed(2); // Scores to euros: 2 numbers after coma
